        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch">
          {nearbyTemples.map((item) => (
            <NextLink
              key={item.slug}
              href={`/${item.slug}`}
              className="group relative flex flex-col items-center text-center p-2 rounded-t-[100px] rounded-b-2xl bg-linear-to-b from-amber-200 via-amber-100 to-amber-300 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
            >
              <div className="relative w-full aspect-4/5 rounded-t-[94px] rounded-b-xl overflow-hidden border border-amber-400/80 bg-stone-900">
                ...
                <div className="absolute bottom-3 left-2 right-2 px-2 py-1">
                  ...
                  >
                    {item.location?.title?.replace(" Location", "") || "India"}
                  </span>
                </div>
              </div>
            </NextLink>
